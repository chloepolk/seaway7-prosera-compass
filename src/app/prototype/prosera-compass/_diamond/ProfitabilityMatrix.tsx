  ...
  if (missions.length === 0) {
    return (
      <div className="flex min-h-[104px] flex-col items-center justify-center rounded-lg border border-dashed border-border/70 bg-muted/20 p-3 text-center">
        ...
        <span className="mt-1 text-[10px] font-medium text-muted-foreground/60">No actions</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex min-h-[104px] flex-col rounded-lg border bg-card p-2.5">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-base font-semibold tabular-nums text-foreground">{formatCurrency(total)}</span>
        <span className="rounded-full bg-muted px-1.5 py-0.5 text-[9px] font-semibold tabular-nums text-muted-foreground">
          {missions.length} action{missions.length !== 1 ? "s" : ""}
        </span>
      </div>
        ...
        {top.map(m => (
          ...
          >
            ...
            <span className="shrink-0 text-[10px] font-semibold tabular-nums text-muted-foreground">
              {formatCurrency(m.realizedValue ?? m.projectedValue)}
            </span>
          </button>
        ))}
        {missions.length > top.length ? (
          <div className="px-1 text-[9px] text-muted-foreground/60">+{missions.length - top.length} more</div>
        ) : null}
      </div>
    </div>
  )
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-foreground">Compass Profitability Matrix</h2>
          <p className="mt-0.5 text-[11px] text-muted-foreground">
            Every recommendation and the dollars behind it — what it does to margin, and how fast.
          </p>
        </div>
        <div className="text-right">
          <div className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground">Total at stake</div>
          <div className="text-lg font-semibold tabular-nums text-foreground">{formatCurrency(grandTotal)}</div>
        </div>
      </div>
        ...
        {OBJECTIVES.map((o, i) => (
          <div key={o.key} className="rounded-lg bg-muted/40 px-2.5 py-1.5">
            <div className="flex items-center justify-between gap-2">
              <span className={cn("text-[12px] font-semibold", o.accent)}>{o.label}</span>
              <span className="text-[12px] font-semibold tabular-nums text-foreground">{formatCurrency(objectiveTotals[i])}</span>
            </div>
        ...
        {HORIZONS.map(h => (
          <React.Fragment key={h.key}>
            <div className="flex flex-col items-center justify-center rounded-lg bg-muted/40 px-1 py-2 text-center">
              ...
              <span className="mt-1 text-[10px] font-bold uppercase tracking-wide text-foreground">{h.label}</span>
              <span className="text-[9px] leading-tight text-muted-foreground">{h.window}</span>
            </div>
            {OBJECTIVES.map(o => (
              <MatrixCell
                key={`${o.key}-${h.key}`}
