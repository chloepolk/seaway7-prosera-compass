...
function fallbackQuality(spec: ComponentSpec): QualityOutput {
  ...
  return {
    intro: "All goods and services supplied under this Invitation to Tender shall comply with the Seaway7 Corporate Quality Assurance Manual (QA-MAN-2026-EPCI, Rev 3.0); deviations require formal dispensation from the Global HSEQ Director.",
    standards: [
...
function fallbackLegal(spec: ComponentSpec): LegalOutput {
  ...
  return {
    governingTerms: "This Invitation to Tender and any subsequent Purchase Order are governed by the Seaway7 Standard Terms and Conditions of Procurement (S7-SCM-TC-2026-v1.0).",
    clauses,
...
function composeItt(
  ...
  return {
    ...
    submissionGuidelines: [
      "Tenders must be submitted electronically via the Seaway7 SCM Portal no later than the submission deadline stated above. Late submissions will not be evaluated.",
...
<body>
  <header>
    <p class="kicker">Seaway7 · Supply Chain Management</p>
    <h1>${esc(itt.title)}</h1>
  ...
  <footer>
    ${esc(itt.ittRef)} · Controlled document — issued via Seaway7 SCM Portal · Drafted and audited against the controlled document register
  </footer>
...
export function TenderStudioPage() {
  ...
  const run = React.useCallback(async (text: string) => {
    ...
    const [techOut, qualOut, legalOut] = await Promise.all([
      postAgent<TechnicalOutput>("/api/seaway7/specialist/technical", { componentId: resolved.id, quantity: qty }, () => fallbackTechnical(resolved, qty), 1200)
        .then(r => { setSpecialists(s => ({ ...s, technical: "done" })); return r }),
      postAgent<QualityOutput>("/api/seaway7/specialist/quality", { componentId: resolved.id }, () => fallbackQuality(resolved), 1600)
        .then(r => { setSpecialists(s => ({ ...s, quality: "done" })); return r }),
      postAgent<LegalOutput>("/api/seaway7/specialist/legal", { componentId: resolved.id }, () => fallbackLegal(resolved), 2000)
