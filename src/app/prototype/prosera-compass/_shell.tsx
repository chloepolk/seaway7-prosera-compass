  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="bottom">
        {DOCUMENTS.length} controlled documents at current revision — register is clean
      </TooltipContent>
    </Tooltip>
  )
}
...
function FindingsPanel() {
  ...
  const phaseLabels = React.useMemo(() => {
    if (agentPhase === "specialists") return ["Pipeline analysis…", "Commercial analysis…", "Supply market signals…"]
    if (agentPhase === "orchestrating") return ["Synthesising findings…", "Cross-referencing documents…", ""]
    if (agentPhase === "verifying") return ["Verifying claims…", "Auditing calculations…", ""]
    return ["", "", ""]
  }, [agentPhase])
...
function AgentStatusStrip() {
  ...
  const phaseText: Record<string, string> = {
    ...
    orchestrating: "Synthesizing intelligence…",
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  const [email, setEmail] = React.useState("j.calder@seaway7.com")
    ...
    <div className="flex min-h-screen w-full items-center justify-center" style={{ background: "linear-gradient(135deg, var(--color-brand-primary) 0%, var(--color-brand-strong) 50%, var(--color-bg-inverse) 100%)" }}>
      ...
      <div className="relative z-10 w-full max-w-[420px] px-6">
        <div className="mb-8 flex flex-col items-center gap-5">
          <img src={PROSERA_LOGO_WHITE} alt="Prosera" className="h-8 w-auto drop-shadow-lg" onError={(e) => { (e.target as HTMLImageElement).style.display = "none" }} />
          <div className="text-center">
            <h1 className="text-2xl font-bold text-white tracking-tight">Prosera Compass</h1>
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl border border-white/10 bg-white/[0.06] p-8 shadow-2xl backdrop-blur-xl">
          <div className="space-y-5">
            <div className="space-y-2">
              ...
              <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@seaway7.com" autoComplete="email" autoFocus className="w-full rounded-lg border border-white/15 bg-white/10 px-4 py-3 text-sm text-white placeholder:text-white/30 outline-none transition focus:border-white/40 focus:bg-white/15 focus:ring-1 focus:ring-white/20" />
            </div>
            <div className="space-y-2">
        ...
        <p className="mt-6 text-center text-[11px] text-white/30">Powered by Prosera</p>
      </div>
    </div>
  )
}
