...
import { toEur } from "../../_i18n/currency"
/* ------------------------------------------------------------------ */
...
export interface S7Document {
  ...
  /** PDF served from /future-energy/ in public. */
  fileName: string
  pages: number
  summary: string
...
export const FAT_TRACEABILITY_CLAUSES = [
  "All critical components must undergo Factory Acceptance Testing (FAT) before shipment; the Supplier shall submit an Inspection and Test Plan (ITP) to Future Energy Quality Control at least 30 days prior to manufacturing.",
...
export const DOCUMENTS: S7Document[] = [
  ...COMPONENT_SPECS.map((spec): S7Document => ({
    ...
    fileName: `/future-energy/${spec.docId}.pdf`,
    pages: 1,
  ...
  })),
  {
    ...
    fileName: "/future-energy/qa-man-2026-epci.pdf",
    pages: 3,
    ...
    fullText: [
      ...
      "1. Scope: QMS applied by Future Energy for all EPCI operations. All Tier 1 and Tier 2 suppliers are strictly mandated to adhere to the applicable standards referenced in this document; deviations require formal dispensation from the Global HSEQ Director.",
  ...
  },
  {
    ...
    fileName: "/future-energy/s7-scm-tc-2026.pdf",
    pages: 3,
  ...
  },
  {
    ...
    pages: 2,
    summary: `Time charter for ${CHARTER.vessel} (${CHARTER.vesselType}) — ${CHARTER.charterPeriod} at EUR ${toEur(CHARTER.hireRate).toLocaleString(activeLocaleTag(), { maximumFractionDigits: 0 })}/day, knock-for-knock liabilities, SOLAS/MARPOL warranty, English law and London arbitration.`,
    fullText: [
      ...
      `Charter period: ${CHARTER.charterPeriod}. Delivery: ${CHARTER.deliveryPort}. Hire: EUR ${toEur(CHARTER.hireRate).toLocaleString(activeLocaleTag(), { maximumFractionDigits: 0 })}/day. Mobilisation: EUR ${toEur(CHARTER.mobilisationFee).toLocaleString(activeLocaleTag(), { maximumFractionDigits: 0 })} lump sum.`,
  ...
  },
  {
    ...
    owner: "Supply Chain Management",
    classification: "Confidential — Internal",
    fileName: "/future-energy/itt-template.pdf",
    pages: 3,
    ...
    fullText: [
      ...
      "Section 1.0 Introduction & Instructions to Tenderers: project overview and submission guidelines. Tenders must be submitted electronically via the Future Energy SCM Portal no later than the submission deadline; late submissions will not be evaluated; clarifications at least 7 days prior to the deadline.",
