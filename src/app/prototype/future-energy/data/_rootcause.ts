...
function analyzePricingGap(
  ...
  for (const [type, jobs] of byType) {
    ...
    if (Math.abs(pctDiff) > 10 && jobs.length >= 2) {
      details.push(`${type}: ${formatActiveUsd(Math.round(entityAvgRev))} avg vs. portfolio ${formatActiveUsd(Math.round(portfolioAvgRev))} (${pctDiff > 0 ? "+" : ""}${pctDiff.toFixed(0)}% across ${jobs.length} jobs)`);
    }
  }
  ...
  return {
    ...
    detail: details.length > 0
      ? details.join(". ")
      : `Weighted pricing delta of ${formatActiveUsd(Math.abs(Math.round(totalDelta)))} across ${valid.length} jobs.`,
  };
}
...
function analyzeLaborCostVariance(
    ...
    if (wageProfile && nvProfile) {
      ...
      if (latest && nvLatest) {
        ...
        if (Math.abs(premium) > 5) {
          benchComp = `BLS data: ${wageProfile.metroArea} HVAC techs avg ${formatActiveUsd(latest.meanAnnualWage )}/yr vs. Metro West-1 ${formatActiveUsd(nvLatest.meanAnnualWage )}/yr (${premium > 0 ? "+" : ""}${premium.toFixed(0)}% structural premium)`;
        }
      }
    }
  }
  ...
  return {
    ...
    detail: `Labor at ${(entityLaborPct * 100).toFixed(1)}% of revenue vs. portfolio benchmark ${(portfolioLaborPct * 100).toFixed(1)}%. ${laborDelta > 0 ? "Excess" : "Savings"} of ${formatActiveUsd(Math.abs(Math.round(laborDelta)))}.`,
...
function analyzeMaterialMarkup(
  ...
  return {
    ...
    detail: `Material + equipment at ${(entityMaterialPct * 100).toFixed(1)}% of revenue vs. portfolio ${(portfolioMaterialPct * 100).toFixed(1)}%. Delta: ${formatActiveUsd(Math.abs(Math.round(materialDelta)))}.`,
    benchmarkComparison: benchComp,
  };
}
...
function analyzeVolumeTicket(
  ...
  return {
    ...
    detail: `Avg ticket ${formatActiveUsd(Math.round(entityAvgTicket))} vs. portfolio ${formatActiveUsd(Math.round(baseline.avgTicket))} (${ticketPctDiff > 0 ? "+" : ""}${(ticketPctDiff * 100).toFixed(0)}%). ${ticketDelta < 0 ? "Small jobs have higher per-unit dispatch and overhead costs." : "Larger jobs amortize fixed costs more effectively."}`,
  };
}
...
function analyzeContractStructure(
  ...
  if (pmMargin != null && pmMargin < 0.15 && pmJobs.length >= 2) {
    ...
    return {
      ...
      detail: `PM contracts at ${(pmMargin * 100).toFixed(1)}% margin across ${pmJobs.length} jobs (${formatActiveUsd(Math.round(pmRevenue))} revenue). Target PM margin: 30-40%.`,
  ...
  if (escalatedJobs.length >= 3 && withinScopeJobs.length >= 1) {
    ...
    if (frictionCost > Math.round(200 * SCALE)) {
      return {
        ...
        detail: `${escalatedJobs.length} jobs exceeded the customer-set NTE threshold, triggering dispatch approval workflow. Escalated jobs average ${avgVisitsEscalated.toFixed(1)} visits vs. ${avgVisitsWithin.toFixed(1)} for within-scope jobs (${visitDelta > 0 ? "+" : ""}${visitDelta.toFixed(1)} visit delta). ${returnTrips} estimated return trips at ~${formatActiveUsd(estTruckRollCost)}/trip = ${formatActiveUsd(frictionCost)} annual friction cost.`,
...
function analyzeRegionalCostPremium(
  ...
  if (regions.length === 1 && regions[0]) {
    ...
    if (wageProfile) {
      ...
      if (latest && national) {
        ...
        benchComp = `${wageProfile.metroArea} HVAC wages ${premium > 0 ? "+" : ""}${premium.toFixed(0)}% vs. national avg (${formatActiveUsd(latest.meanAnnualWage )} vs. ${formatActiveUsd(national.meanAnnualWage )}). ${costDelta > 0 ? "Structural cost premium should be reflected in regional pricing." : ""}`;
      }
    }
  }
  ...
  return {
    ...
    detail: `Cost at ${(entityCostPct * 100).toFixed(1)}% of revenue vs. portfolio-adjusted expectation ${(expectedCostPct * 100).toFixed(1)}%. Regional operations in ${regionStr} carry a ${costDelta > 0 ? "premium" : "discount"} of ${formatActiveUsd(Math.abs(Math.round(costDelta)))}.`,
    benchmarkComparison: benchComp,
  };
}
...
export function analyzeEntity(
  ...
  const topLever = drivers.length > 0
    ? `${drivers[0].driver}: ${formatActiveUsd(Math.abs(drivers[0].dollarImpact))} ${drivers[0].direction === "drag" ? "recoverable" : "advantage"}`
...
function computeNTEPrescriptions(
  ...
  for (const [jobType, typeJobs] of byType) {
    ...
    actions.push({
      action: `${roleForLever("Dispatch re-auth workflow")}: cut ${jobType} NTE escalation loops from ${typeEscalated.length}/${typeJobs.length} to ${typeEscalated.length - escalationsEliminated}/${typeJobs.length} jobs/yr via single-queue re-auth — saves ${formatActiveUsd(annualSavings)}/yr in truck-roll overhead (${escalationsEliminated} loops × ${formatActiveUsd(estTruckRollCost)} × ${(returnTripRate * 100).toFixed(0)}% return-trip rate)`,
      lever: "Dispatch re-auth workflow",
      currentValue: `${formatActiveUsd(Math.round(avgNTE))} customer-set NTE, ${typeEscalated.length}/${typeJobs.length} jobs escalate (${(typeEscalated.length / typeJobs.length * 100).toFixed(0)}% escalation rate)`,
      ...
      math: `Escalated jobs avg ${avgVisitsEscalated.toFixed(1)} visits vs. within-scope ${avgVisitsWithin.toFixed(1)} visits. ${escalationsEliminated} eliminated escalations × ${formatActiveUsd(estTruckRollCost)} truck roll × ${(returnTripRate * 100).toFixed(0)}% return-trip rate = ${formatActiveUsd(annualSavings)}/yr dispatch overhead savings`,
...
function computeLaborRatePrescriptions(
  ...
  for (const [jobType, typeJobs] of byType) {
    ...
    actions.push({
      action: `${roleForLever("Labor billing rate")}: raise ${jobType} billing from ${formatActiveUsd(Math.round(currentBillingRate))}/hr to ${formatActiveUsd(targetBillingRate)}/hr (${LABOR_MULTIPLIER_TARGET}× tech cost) on ~${typeJobs.length} jobs/yr — ${formatActiveUsd(annualRecovery)}/yr margin recovery at ${avgHoursPerJob.toFixed(1)} hrs/job avg`,
      lever: "Labor billing rate",
      currentValue: `${formatActiveUsd(Math.round(currentBillingRate))}/hr billing (${currentMultiplier.toFixed(1)}× tech cost of ${formatActiveUsd(Math.round(avgCostRate))}/hr)`,
      targetValue: `${formatActiveUsd(targetBillingRate)}/hr billing (${LABOR_MULTIPLIER_TARGET}× multiplier)`,
      math: `Tech cost ${formatActiveUsd(Math.round(avgCostRate))}/hr × ${LABOR_MULTIPLIER_TARGET}× = ${formatActiveUsd(targetBillingRate)}/hr target. At current volume (~${typeJobs.length} jobs/yr, ${avgHoursPerJob.toFixed(1)} hrs/job avg), projected annual uplift: ${formatActiveUsd(Math.round(rateUplift))}/hr × ${totalHours.toFixed(0)} hrs = ${formatActiveUsd(annualRecovery)}/yr`,
...
function computeMaterialMarkupPrescriptions(
  ...
  actions.push({
    action: `${roleForLever("Material markup")}: lift material markup from ${(currentMarkupPct * 100).toFixed(1)}% to ${(targetMarkupPct * 100).toFixed(0)}% of revenue on ~${matJobs.length} ${primaryType} jobs/yr — closes ${(markupGapPct * 100).toFixed(1)}pt gap for ${formatActiveUsd(annualRecovery)}/yr uplift`,
    lever: "Material markup",
    currentValue: `${(currentMarkupPct * 100).toFixed(1)}% material cost as % of revenue (${formatActiveUsd(Math.round(totalMatCost))} on ${formatActiveUsd(Math.round(totalRevenue))})`,
    ...
    math: `Gap: ${(markupGapPct * 100).toFixed(1)} pts. At current volume (~${matJobs.length} jobs/yr, ${formatActiveUsd(Math.round(totalRevenue))} revenue), applying this markup going forward = ${formatActiveUsd(annualRecovery)}/yr projected uplift`,
    projectedAnnualUplift: annualRecovery,
...
function computePricingGapPrescriptions(
  ...
  for (const [jobType, typeJobs] of byType) {
    ...
    actions.push({
      action: `${roleForLever("Job ticket pricing")}: reprice ${jobType} quotes from ${formatActiveUsd(Math.round(entityAvgTicket))} to ${formatActiveUsd(targetTicket)} (95% of portfolio ${formatActiveUsd(Math.round(portfolioAvgTicket))}) on ~${typeJobs.length} jobs/yr — ${formatActiveUsd(annualRecovery)}/yr uplift, margin ${currentMargin}% → ${targetMargin}%`,
      lever: "Job ticket pricing",
      currentValue: `${formatActiveUsd(Math.round(entityAvgTicket))} avg ticket (${currentMargin}% margin) vs. portfolio ${formatActiveUsd(Math.round(portfolioAvgTicket))}`,
      targetValue: `${formatActiveUsd(targetTicket)} (95% of portfolio avg, ${targetMargin}% margin)`,
      math: `${formatActiveUsd(Math.round(perJobUplift))} uplift/job. At current volume (~${typeJobs.length} jobs/yr at ${formatActiveUsd(Math.round(entityAvgCost))} avg cost), projected annual uplift: ${formatActiveUsd(annualRecovery)}/yr`,
      projectedAnnualUplift: annualRecovery,
      jobCount: typeJobs.length,
      jobType,
      confidence: typeJobs.length >= 5 ? "high" : "medium",
...
export function buildPricingBandInsights(
  ...
  const fmtUsd = (n: number) => formatActiveUsd(n);
