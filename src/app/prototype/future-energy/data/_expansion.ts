...
function buildMarketSignals(region: Region, sc: StrategyScorecard, regionAgg: RegionAggregate | undefined): MarketSignal[] {
  ...
  if (permits.length > 0) {
    ...
    signals.push({
      ...
      value: `${latestPermits.toLocaleString(activeLocaleTag())} units (${topPermit.metroArea})`,
      implication: `${latestPermits.toLocaleString(activeLocaleTag())} permits/yr in ${topPermit.metroArea} (dataset activity level: ${topPermit.constructionActivityLevel}).`,
    });
...
function buildActions(
  ...
  if (strategy === "invest" || strategy === "expand") {
    ...
    if (topPermit && topPermit.constructionActivityLevel === "high") {
      const latestPermits = topPermit.snapshots[topPermit.snapshots.length - 1]?.totalPermits ?? 0;
      actions.push({
        action: `Corp Dev Lead: target M&A in ${topPermit.metroArea} — ${latestPermits.toLocaleString(activeLocaleTag())} Census permits/yr (high activity) converts install pipeline to recurring service; model 4–6× EBITDA on sub-scale sellers`,
        lever: "M&A",
        ...
        math: `Permit pipeline = ${latestPermits.toLocaleString(activeLocaleTag())} units/yr (${topPermit.metroArea}, Census BPS) × install-to-service conversion rate`,
