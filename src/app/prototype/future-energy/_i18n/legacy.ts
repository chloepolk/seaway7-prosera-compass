...
export function localizeLegacyCopy(text: string, locale: Locale): string {
  ...
  if (/^Develop Stars candidates\b/.test(text)) return "Développer des candidats Stars en optimisant le mix de services."
  if (/^Streamline re-auth workflow\b/.test(text)) return "Simplifier le workflow de réautorisation pour réduire les boucles d’approbation et les déplacements inutiles."
  if (/^NTE escalation levels are low\b/.test(text)) return "Les escalades NTE sont faibles ; aucune friction de réautorisation significative n’est détectée."
  ...
  if (/^Win-rate curve\b/.test(text)) return "Courbe du taux de réussite par tranche de prix ; la zone optimale maximise la conversion."
