/* ------------------------------------------------------------------ */
/*  Agent context builders — Future Energy Meridian OWF procurement          */
/*                                                                     */
...
import { ALL_BIDS, bidsForPackage, packagesWithBids, type BidInput } from "../data/future-energy/_bids"
import {
  evaluateBids,
...
export function scoringModelSummary() {
  return {
    ...
    legalRule: `Knock-for-knock acceptance 10 + standard Future Energy warranty acceptance 10; warranty under ${STANDARD_WARRANTY_MONTHS} months post-install → −${WARRANTY_SHORTFALL_PENALTY} (floored at 0)`,
...
export function buildPortfolioContext(_data: ComputedData, drill: DrillState): Record<string, unknown> {
  return {
    ...
    processRules: {
      ...
      submissionChannel: "Future Energy SCM Portal — late submissions are not evaluated",
